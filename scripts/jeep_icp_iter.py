...
from slam_04_d_apply_transform_question import\
     estimate_transform, apply_transform, concatenate_transform#, correct_pose
...
class JeepICP():
    def __init__(self):
        ...
        self.odom_x = 6.0 #6.0
        self.odom_y = 0.5 #-8.0
        self.odom_theta = -0.04 #Try -0.03 w.r.t. map alignment (landmarks made square above)
        ...
        if(self.cloud_count <=1): #TEMPORARY JUST TO TESTS MULTIPLE ITERATIONS WITH ONE CLOUD
            ...
            try:
                ...
                if(len(left_list) >= self.min_point_pairs):
                    ...
                    for j in xrange(5):
                        rk = 0
                        new_left_list = [apply_transform(cum_trafo, p) for p in left_list]
                        left_list = []
                        right_list = []
                        ref_type_list = [] # currently not used
                        for pk,left in enumerate(new_left_list):
                            rk, ref_pt, ref = self.find_match(rk, left[0], left[1])
                            if(not ref == None):
                    ...
                    if( (new_dist_sum < cur_dist_sum) ): #and (abs(cur_dist_sum - self.prev_eval_dist_sum) < 30.) ):
                        self.prev_eval_dist_sum = cur_dist_sum
