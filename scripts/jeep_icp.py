...
class JeepICP():
    def __init__(self):
        ...
        self.landmarks.append(MapRef('point',18.1, -5.3))
                        ...
                        if( (nSegPts == 1 or self.get_abs_angle_diff(ang,ang_cur) < angle_eps  )
                           and sub_seg_dist_sqd < 8.**2): # and segPtB[3]-segPt1[3] <= 2 # not available with bag data from scanse
                            ...
                            if(nSegPts >= self.min_nSegPts):
                                line_segs.append(np.array([ [segPtA[0],segPtA[1]], [segPt1[0],segPt1[1]] ]) )
                                print "FOUND LINE SEGMENT, nSegPts: ", nSegPts, "angle: ", self.angle(segPtA,segPt1)
                                print line_segs[-1]
